...
U2 SENSOR_CHANGE_CHECK_DELAY = 30;
...
public:
  ...
  void setPhotoState(int photoState, U64 timeStamp)
  {
    ...
    if (this->currentSensorState == PHOTO_ON && photoState == PHOTO_OFF) {
        U64 duration = timeStamp - this->currentTimestamp;  // ✅ คำนวณจาก timestamp จริง
  ...
  bool shouldSendCountUpFlg(U64 now)
  {
    ...
    // ใหม่: เช็คว่า ON ต้องค้างมากกว่า 100ms ถึงจะนับ
    U64 tsDiff = now - this->previousTimestamp;
    if (tsDiff < 50) {
      DEBUG_SERIAL.printf("⏱️ COUNT_UP blocked: ON too short (%.1f ms)\n", tsDiff * 1.0);
      return false;
    }
...
void loop()
{
  ...
  // ✅ ปริ้นเวลาทุก 1 นาที (UTC+7)
  nowTs = initialTimestamp + millis();
  static U64 lastReadablePrint = 0;
  if (millis() - lastReadablePrint >= 60000) {
    lastReadablePrint = millis();
    time_t nowSec = (nowTs / 1000) + (7 * 3600);  // UTC+7
  ...
  // ✅ ตรวจ heap ค้าง → รีบูต
  
  if (millis() - lastHeapCheck > 30000) {
    int currentHeap = ESP.getFreeHeap();
    if (currentHeap < 2500 && abs(currentHeap - lastHeap) < 100) {
      DEBUG_SERIAL.println("💣 Heap stuck or dangerously low. Rebooting...");
      ESP.restart();
    }
    lastHeap = currentHeap;
    lastHeapCheck = millis();
  ...
  for (U1 cnt = 0; cnt < allSlotCount; cnt++) {
    ...
    if (slot->isSensorStateChanged(photoState, currentTimestamp)) {
      ...
      if (photoState == PHOTO_ON) {
        // ✅ Filter False ON เฉพาะ SEND_TYPE
        if (transferType == SLOT_SEND_TYPE && tsDiff < 300) {
          DEBUG_SERIAL.printf("🧪 [False ON Filter] Sensor [%s %d,%d] ON < 300ms → Ignore (%.1f ms)\n",
              flowrackName.c_str(), row, col, tsDiff * 1.0);
          continue;  // ❌ ไม่ส่ง COUNT_UP
        }

        if (transferType == SLOT_SEND_TYPE && slot->shouldSendCountUpFlg(currentTimestamp)) {
          if (!slot->getIsSendingCountUp() && slot->canPostNow() && activeRequests < 4) {
            
            // ✅ เพิ่มบรรทัดนี้เพื่อวัด delay ก่อนส่ง COUNT_UP
            U64 delayToPost = currentTimestamp  - slot->getCurrentTimestamp();
            DEBUG_SERIAL.printf("⏱️ Delay before COUNT_UP post: %llu ms (%s [%d,%d])\n",
                                delayToPost,
                                flowrackName.c_str(), row, col);
            ...
            DEBUG_SERIAL.printf("⏸️ [COUNT_UP blocked] Pacing or already sending: %s [%d,%d] | canPost=%d | sending=%d | activeReq=%d\n",
                flowrackName.c_str(), row, col,
                slot->canPostNow(), slot->getIsSendingCountUp(), activeRequests);
          }
        }
        else {
          DEBUG_SERIAL.printf("❌ [COUNT_UP skipped] Not eligible: %s [%d,%d] | State=%d | tsDiff=%.1fms\n",
              flowrackName.c_str(), row, col,
              slot->getCurrentSensorState(),
              (currentTimestamp - slot->getPreviousTimestamp()) * 1.0);
        }
  ...
  if (allowRoundRobin && allSlotCount > 0) {
    ...
    for (int loop = 0; loop < allSlotCount; loop++) {
      ...
      if (slot->canPostNow() && activeRequests < 4) {
        ...
        // ✅ กรอง HOLD ที่ ON แป๊บเดียว
        if (state == HOLD && currentTimestamp - slot->getPreviousTimestamp() < SENSOR_HOLD_CHECKED) {
          DEBUG_SERIAL.printf("⚠️ Skip RoundRobin HOLD: %s [%d,%d] ON too short (%.1fs)\n",
              flowrackName.c_str(), row, col, (currentTimestamp - slot->getPreviousTimestamp()) / 1000.0);
          continue;
        }
...
void asyncPostHTTP(String flowrack, RequestCounterState state, int row, int col, bool allowRetry) {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (failedRequests.size() > 100) failedRequests.clear();
    return;
  }
  ...
  // 🔁 สำเนาตัวแปรเข้า lambda
  request->onReadyStateChange([request, jsonPayload, flowrack, row, col, allowRetry, state](void *arg, AsyncHTTPRequest *req, int readyState) {
    if (readyState != readyStateDone) return;
    activeRequests--;
    int code = req->responseHTTPcode();
    DEBUG_SERIAL.printf("✅ %s[%d,%d] s:%d → %d\n", flowrack.c_str(), row, col, state, code);
  ...
  DEBUG_SERIAL.printf("📤 POST → %s[%d,%d]: %s\n", flowrack.c_str(), row, col, shortPayload.c_str());
  ...
  if (activeRequests < 2) delay(10 + random(20));
  yield();
}
...
void retryFailedRequests() {
  ...
  request->onReadyStateChange([request, packet](void *arg, AsyncHTTPRequest *req, int readyState) mutable {
    ...
    if (code == 200) {
      ...
      DEBUG_SERIAL.printf("✅ RETRY OK: %d\n", code);
      DEBUG_SERIAL.println("===================================");
  ...
  if (request->open("POST", serverPostURL.c_str())) {
    request->setReqHeader("Content-Type", "application/json");
    if (activeRequests < 2) delay(10 + random(10));
    yield();
