...
public:
  bool alreadySentHold = false;
  bool alreadySentNoHold = false;
  bool getAlreadySentHold() { return alreadySentHold; }
  void setAlreadySentHold(bool value) { alreadySentHold = value; }

  bool getAlreadySentNoHold() { return alreadySentNoHold; }
  void setAlreadySentNoHold(bool value) { alreadySentNoHold = value; }
  ...
  bool shouldSendHoldFlg(U64 currentTimestamp)
  {
    ...
    {
        //DEBUG_SERIAL.println("❌ Sensor is OFF. Skipping HOLD request.");
        return false;
    }

    if (this->requestState == REQUEST_HOLD)
    {
        //DEBUG_SERIAL.println("⏳ HOLD already sent. Skipping duplicate request.");
        return false;
    }
  ...
  bool shouldSendReleaseHoldFlg(U64 currentTimestamp)
  {
    if (this->currentSensorState == PHOTO_ON) 
    {
        return false;  // ✅ ถ้ายังมีของอยู่ ไม่ต้องส่ง NO_HOLD
    }

    U64 timeDiff = currentTimestamp - this->currentTimestamp;
    
    if (this->requestState == REQUEST_HOLD && timeDiff > SENSOR_HOLD_CHECKED)
    {
        DEBUG_SERIAL.println("✅ Hold timeout reached! Sending NO_HOLD.");
        this->setRequestState(NO_REQUEST);  // ✅ รีเซ็ตสถานะหลังจากส่ง NO_HOLD
        return true;
    }
...
void checkFullRealTime()
{
    ...
    for (U1 cnt = 0; cnt < allSlotCount; cnt++)
    {
        ...
        if (SlotObj->getSlotTransferType() == SLOT_RECEIVE_TYPE)
        {
            ...
            if (SlotObj->shouldSendHoldFlg(currentMillis) && !SlotObj->getAlreadySentHold())
            {
                ...
                SlotObj->setAlreadySentHold(true);
                SlotObj->setAlreadySentNoHold(false);
            }
            else if (SlotObj->shouldSendReleaseHoldFlg(currentMillis) && !SlotObj->getAlreadySentNoHold())
            {
                DEBUG_SERIAL.println("✅ Slot is free! Sending NO_HOLD.");
                asyncPostHTTP(flowrackName, NO_HOLD, row, col);
                SlotObj->setAlreadySentNoHold(true);
                SlotObj->setAlreadySentHold(false);
            }
        }
    }
}
...
void handlePhotoState(U2 input_channel_1, U2 input_channel_2, U64 currentTimestamp)
{
    ...
    for (U1 cnt = 0; cnt < allSlotCount; cnt++)
    {
        ...
        else if (photoState == PHOTO_ON)
        {
            if (transferType == SLOT_RECEIVE_TYPE && SlotObj->shouldSendHoldFlg(currentTimestamp))
            {
                asyncPostHTTP(flowrackName, HOLD, row, col);
            }
            else if (transferType == SLOT_SEND_TYPE && SlotObj->shouldSendCountUpOnHoldSendSlot(currentTimestamp))
            {
                asyncPostHTTP(flowrackName, COUNT_UP, row, col);
            }
        }
...
// ✅ ฟังก์ชันส่ง HTTP Request ที่ยังค้างอยู่
void retryFailedRequests() {
    if (failedRequests.empty()) return; // ไม่มี Request ค้าง → ออกจากฟังก์ชัน

    DEBUG_SERIAL.print("🔄 Retrying failed requests: ");
    DEBUG_SERIAL.println(failedRequests.size());

    std::vector<String> tempQueue = failedRequests;  // คัดลอก Request ที่ค้างไว้
    failedRequests.clear();  // ล้าง Queue เดิมก่อน เพื่อป้องกันการวนซ้ำถ้าเน็ตหลุดอีก
    ...
    for (String payload : tempQueue) {
        ...
        request->onReadyStateChange([request, payload](void *arg, AsyncHTTPRequest *req, int readyState) {
            if (readyState == readyStateDone) {
                if (req->responseHTTPcode() != 200) {  
                    ...
                    failedRequests.push_back(payload);  // ถ้ายังไม่สำเร็จ → ใส่กลับไปใน queue
                }
                delete request;
            }
        });
        ...
        delay(500);  // ✅ เพิ่มดีเลย์ 500ms เพื่อลดภาระ ESP และ Network
