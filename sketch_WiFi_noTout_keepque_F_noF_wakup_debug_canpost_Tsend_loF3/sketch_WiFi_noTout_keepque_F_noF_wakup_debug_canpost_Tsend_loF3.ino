  ...
  for (U1 cnt = 0; cnt < allSlotCount; cnt++) {
    ...
    if (slot->isSensorStateChanged(photoState, currentTimestamp)) {
      ...
      if (photoState == PHOTO_ON) {
        ...
        if (transferType == SLOT_RECEIVE_TYPE && slot->canPostNow() && activeRequests < 3) {
          slot->updateLastPostTime();
...
void asyncPostHTTP(String flowrack, RequestCounterState state, int row, int col)
{
  ...
  if (activeRequests >= 4) {  // 💡 จำกัดพร้อมกันไม่เกิน 3
  ...
  {
    request->setTimeout(5000); // ตั้ง timeout 3 วินาที
    ...
    delay(30 + random(50)); // 💡 หน่วงเบา ๆ ให้ TCP stack ระบาย connection เก่า
...
void retryFailedRequests() {
  ...
  U64 backoffDelay = 5000 + packet.retryCount * 5000 + random(500);
  if (currentMillis - lastRetryTime < backoffDelay) {
    DEBUG_SERIAL.println("⏳ [Retry] Backoff active. Skip this round.");
    return;
  }

  if (WiFi.status() != WL_CONNECTED) {
    DEBUG_SERIAL.println("🚫 [Retry] WiFi not connected.");
    return;
  }

  if (activeRequests >= 4) {
    DEBUG_SERIAL.println("🚦 [Retry] Skip retry: too many activeRequests.");
    return;
  }
  ...
  if (packet.retryCount >= 3) {
    DEBUG_SERIAL.printf("❌ Max retry reached for: %s [%d,%d]. Dropping.\n", flowrack.c_str(), row, col);
    return;
  }

  DEBUG_SERIAL.printf("♻ Retrying: %s [%d,%d] Attempt: %d\n", flowrack.c_str(), row, col, packet.retryCount + 1);
  ...
  request->onReadyStateChange([request, packet, flowrack, row, col](void *arg, AsyncHTTPRequest *req, int readyState) mutable {
    ...
    if (code == 200) {
      ...
      DEBUG_SERIAL.printf("🚫 Server rejected retry [%s] with HTTP %d\n", flowrack.c_str(), code);
      // Don't retry again
    }
  ...
  int delayMs = 30 + random(70);
  DEBUG_SERIAL.printf("⏱ Delay before retry: %d ms\n", delayMs);
  delay(delayMs);
  yield();
