...
public:
  ...
  bool shouldSendReleaseHoldFlg(U64 currentTimestamp)
  {
    ...
    // 📌 ถ้า MCP23S17 ไม่ถูกพบ → ไม่ต้องส่ง NO_HOLD
    if (inputchip1.digitalRead() == 0xFFFF && inputchip2.digitalRead() == 0xFFFF) 
    {
        DEBUG_SERIAL.println("⚠ ERROR: MCP23S17 not detected! Skipping NO_HOLD...");
        return false;
    }
    ...
    DEBUG_SERIAL.println(timeDiff);
    DEBUG_SERIAL.print("📌 Request State: ");
    DEBUG_SERIAL.println(this->requestState);

    // 📌 ถ้า ESP เปิดขึ้นมาแล้ว PHOTO_OFF ตั้งแต่แรก → รีเซ็ต requestState
    if (this->requestState == REQUEST_HOLD && this->currentSensorState == PHOTO_OFF)
    {
        DEBUG_SERIAL.println("🔄 ESP Startup: Clearing FULL state.");
...
void handlePhotoState(U2 input_channel_1, U2 input_channel_2, U64 currentTimestamp)
{
  ...
  // 📌 ตรวจสอบว่ามี MCP23S17 เชื่อมต่อหรือไม่
      ...
      else
      {
        ...
        else if (transferType == SLOT_RECEIVE_TYPE)
        {
            if (SlotObj->shouldSendReleaseHoldFlg(currentTimestamp))
            {
                ...
                // ✅ รีเซ็ต requestState หลังจากส่ง NO_HOLD เพื่อป้องกันการส่งซ้ำ
                SlotObj->setRequestState(NO_REQUEST);
            }
        }
      }
