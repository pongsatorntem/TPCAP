...
byte flowrackIp[4] = {192, 168, 1, 207};  // สุ่มเลขไม่ให้ซ้ำ
...
const int MAX_HTTP_MINUS4_BEFORE_REBOOT = 3;
...
void loop()
{
  ...
  // ✅ ตรวจ activeRequests ค้างนานเกิน 15 วินาที
  ...
  if (activeRequests > 0) {
    if (activeRequests != lastKnownActiveRequests) {
      ...
      lastHttpStateChange = millis(); // มีการเปลี่ยนแปลง → รีเซ็ต
    } else if (millis() - lastHttpStateChange > 15000) {
      DEBUG_SERIAL.println("🧨 WARNING: activeRequests stuck. Forcing clear...");
      activeRequests = 0;  // ✅ ไม่รีเซ็ตทั้งเครื่อง แต่ล้างตัวนับเอง
...
U64 lastFullCheck = 0;  // global timer สำหรับ full-check ทุก 10 วินาที
...
void handlePhotoState(U2 input_channel_1, U2 input_channel_2, U64 currentTimestamp)
{
    ...
    static std::map<int, int> retryOnStuckCount;

    bool shouldCheckFullNow = (currentTimestamp - lastFullCheck >= 60000);
    if (shouldCheckFullNow) lastFullCheck = currentTimestamp;
    ...
    for (U1 cnt = 0; cnt < allSlotCount; cnt++)
    {
        ...
        if (transferType == SLOT_SEND_TYPE && photoState == PHOTO_ON) {
            ...
            if (now - lastRapidTime[rIndex] < 1000) {
                rapidOnCount[rIndex]++;
                if (rapidOnCount[rIndex] > 3) {
                    if (now - SlotObj->getCurrentTimestamp() < 5000) {
                        DEBUG_SERIAL.printf("🚫 Rapid ON detected at rIndex %d. Skipping this round.\n", rIndex);
                        continue;
                    } else {
                        DEBUG_SERIAL.printf("⚠️ ON stuck >5s → allow retry at rIndex %d\n", rIndex);
                    }
                }
            } else {
        ...
        if (SlotObj->getIsSendingCountUp() && millis() - SlotObj->getLastPostTime() > 5000) {
            if (retryOnStuckCount[rIndex] < 3) {
                DEBUG_SERIAL.printf("⚠️ ON stuck >5s → allow retry at rIndex %d (retry #%d)\n", rIndex, retryOnStuckCount[rIndex] + 1);
                SlotObj->setIsSendingCountUp(false);
                retryOnStuckCount[rIndex]++;
            } else {
                DEBUG_SERIAL.printf("🚫 ON stuck retry limit reached at rIndex %d. Not retrying.\n", rIndex);
            }
        }
        ...
        if (SlotObj->isSensorStateChanged(photoState, currentTimestamp)) {
            ...
            if (photoState == PHOTO_ON) {
                DEBUG_SERIAL.printf("Index: %d changed to: on\n", rIndex);
                if (transferType == SLOT_SEND_TYPE && SlotObj->shouldSendCountUpFlg()) {
                    if (!SlotObj->getIsSendingCountUp() && SlotObj->canPostNow() && activeRequests < 3) {
                        SlotObj->setIsSendingCountUp(true);
                        SlotObj->updateLastPostTime();
                        DEBUG_SERIAL.println("🔼 Sending COUNT_UP!");
                        asyncPostHTTP(flowrackName, COUNT_UP, row, col);
                    }
                }

                if (transferType == SLOT_RECEIVE_TYPE && SlotObj->canPostNow() && activeRequests < 3) {
                    SlotObj->updateLastPostTime();
                ...
                if (transferType == SLOT_RECEIVE_TYPE && SlotObj->shouldSendReleaseHoldFlg(currentTimestamp)) {
                    if (!SlotObj->getIsSendingNoHold() && SlotObj->canPostNow() && activeRequests < 3) {
                        SlotObj->setIsSendingNoHold(true);
                        SlotObj->setIsSendingHold(false);
                        SlotObj->updateLastPostTime();
                        asyncPostHTTP(flowrackName, NO_HOLD, row, col);
                    }
                }
        ...
        if (transferType == SLOT_RECEIVE_TYPE && SlotObj->shouldSendHoldFlg(currentTimestamp)) {
            if (!SlotObj->getIsSendingHold() && SlotObj->canPostNow() && activeRequests < 3) {
                SlotObj->setIsSendingHold(true);
                SlotObj->setIsSendingNoHold(false);
                SlotObj->updateLastPostTime();
                asyncPostHTTP(flowrackName, HOLD, row, col);
            }
        }

        // ✅ เพิ่ม logic ใหม่: เช็ค FULL/NOFULL ทุก 1 นาที พร้อม log rack-styled
        if (shouldCheckFullNow && transferType == SLOT_RECEIVE_TYPE) {
            if (col == 0) {
                DEBUG_SERIAL.printf("rack %s ", flowrackName.c_str());
            }

            const char* statusText = (photoState == PHOTO_ON) ? "Full" : "NoFull";
            DEBUG_SERIAL.printf("(%d,%d) = %s\n", row, col, statusText);

            if (SlotObj->canPostNow() && activeRequests < 3) {
                RequestCounterState state = (photoState == PHOTO_ON) ? HOLD : NO_HOLD;
                asyncPostHTTP(flowrackName, state, row, col);
                SlotObj->updateLastPostTime();
            } else {
                DEBUG_SERIAL.printf("⏳ [Periodic] Skipped due to pacing or activeRequests: %s [%d,%d]\n", flowrackName.c_str(), row, col);
            }
        }
...
void asyncPostHTTP(String flowrack, RequestCounterState state, int row, int col)
{
  ...
  if (ESP.getFreeHeap() < 10000) {
    DEBUG_SERIAL.println("🧯 Heap too low. Skipping HTTP request.");
    return;
  }
  ...
  {
    request->setTimeout(3000); // ตั้ง timeout 3 วินาที
    ...
    request->onReadyStateChange([request, jsonPayload, flowrack, row, col](void *arg, AsyncHTTPRequest *req, int readyState) {
      DEBUG_SERIAL.print("⏳ HTTP state changed → ");
      DEBUG_SERIAL.print(readyState);
      DEBUG_SERIAL.print(" | HTTP code (now): ");
      DEBUG_SERIAL.println(req->responseHTTPcode());
      ...
      if (readyState == readyStateDone) {
        ...
        if (activeRequests > 0) activeRequests--;   // 📌 ลดตัวนับเมื่อเสร็จ
        ...
        if (code == -4) {
          ...
          DEBUG_SERIAL.println("❗ CONNECTION FAILED → HTTP -4 count: " + String(httpErrorMinus4Count));

          if (httpErrorMinus4Count >= MAX_HTTP_MINUS4_BEFORE_REBOOT) {
            DEBUG_SERIAL.println("🛑 Too many HTTP -4 errors. Rebooting...");
            delay(1000);
            ESP.restart();
          }
        } else {
          httpErrorMinus4Count = 0; // ✅ Reset เมื่อเจอ 200 หรือ error อื่น
        }
        ...
        if (code != 200) {
          if (code == -4 || code == -1 || code == 0) {
            DEBUG_SERIAL.println("⚠ ERROR: Network issue. Retrying...");
            failedRequests.push_back({jsonPayload, 0});
          } else {
            DEBUG_SERIAL.println("🚫 Server error " + String(code) + ". Not retrying.");
          }
        }
    ...
    if (!success) {
        ...
        delay(20);  // 💡 ปล่อยเวลาให้ TCP ปิดก่อนลบทิ้ง
        delete request;
    }
  }
  else
...
void retryFailedRequests() {
  ...
  if (currentMillis - lastRetryTime < 3000 + random(200)) return; // ✅ เพิ่ม jitter
  ...
  request->onReadyStateChange([request, packet, flowrack, row, col](void *arg, AsyncHTTPRequest *req, int readyState) mutable {
    ...
    if (code == -4) {
      httpErrorMinus4Count++;
      DEBUG_SERIAL.println("❗ Retry HTTP -4 count: " + String(httpErrorMinus4Count));

      if (httpErrorMinus4Count >= MAX_HTTP_MINUS4_BEFORE_REBOOT) {
        DEBUG_SERIAL.println("🛑 Too many retry HTTP -4 errors. Rebooting...");
        delay(1000);
        ESP.restart();
      }
    } else {
  ...
  // ✅ สุ่ม delay เล็กน้อยเพื่อกระจายโหลด retry
  int delayMs = 30 + random(70);  // 30–99ms
  DEBUG_SERIAL.print("⏱ Delay before retry: ");
