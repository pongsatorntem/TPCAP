            ...
            if (now - lastRapidTime[rIndex] < 1000) {
                ...
                if (rapidOnCount[rIndex] > 3) {
                    if (now - SlotObj->getCurrentTimestamp() < 5000) {
                        ...
                        continue;
                    } else if (retryOnStuckCount[rIndex] < 3) {
                    ...
                    if (!SlotObj->getIsSendingCountUp()) {
                        if (SlotObj->canPostNow()) {
                            if (activeRequests < 3) {
                                ...
                                retryOnStuckCount[rIndex] = 99; // ✅ ป้องกันส่งซ้ำรัวๆ
                            } else {
                                DEBUG_SERIAL.println("⏳ Skipped COUNT_UP due to activeRequests limit");
                            }
                        } else {
                            DEBUG_SERIAL.println("⏳ Skipped COUNT_UP due to pacing");
                        }
                    } else {
                ...
                if (transferType == SLOT_RECEIVE_TYPE)
                {
                    ...
                    if (SlotObj->canPostNow()) {
                        if (activeRequests < 3) {
                            ...
                            DEBUG_SERIAL.println("⏳ Skipped ITEM_IN due to activeRequests limit");
                        }
                    } else {
                        DEBUG_SERIAL.println("⏳ Skipped ITEM_IN due to pacing");
                    }
                }
            }
            else
                    ...
                    if (!SlotObj->getIsSendingNoHold()) {
                        ...
                        if (SlotObj->canPostNow()) {
                            if (activeRequests < 3) {
                                ...
                                DEBUG_SERIAL.println("⏳ Skipped NO_HOLD due to activeRequests limit");
                            }
                        } else {
                            DEBUG_SERIAL.println("⏳ Skipped NO_HOLD due to pacing");
                        }
                    } else {
            ...
            if (!SlotObj->getIsSendingHold()) {
                ...
                if (SlotObj->canPostNow()) {
                    if (activeRequests < 3) {
                        ...
                        DEBUG_SERIAL.println("⏳ Skipped HOLD due to activeRequests limit");
                    }
                } else {
                    DEBUG_SERIAL.println("⏳ Skipped HOLD due to pacing");
                }
            } else {
