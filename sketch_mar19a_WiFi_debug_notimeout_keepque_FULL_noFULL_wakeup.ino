...
void asyncPostHTTP(String flowrack, RequestCounterState state, int row, int col)
{
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    if (request->readyState() == readyStateUnsent || request->readyState() == readyStateDone)
    {
      ...
      request->onReadyStateChange([request, jsonPayload](void *arg, AsyncHTTPRequest *req, int readyState) {
          if (readyState == readyStateDone) {
              DEBUG_SERIAL.println("📥 Server responded with HTTP " + String(req->responseHTTPcode()));


              int code = req->responseHTTPcode();
              DEBUG_SERIAL.print("📡 HTTP Code: ");
              DEBUG_SERIAL.println(code);                 // ✅ แสดง HTTP Status
    ...
    else
    {
      ...
      failedRequests.push_back(jsonPayload); // เก็บไว้ retry
...
void retryFailedRequests() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    DEBUG_SERIAL.println("⚠ Cannot retry now, WiFi disconnected.");
    return;
  }
  
  std::vector<RetryPacket> tempQueue = failedRequests;
  failedRequests.clear();

  for (RetryPacket packet : tempQueue) {
    if (packet.retryCount >= 3) {
      DEBUG_SERIAL.println("❌ Max retry reached. Dropping packet.");
      continue;
    }

    DEBUG_SERIAL.print("[RETRY] Sending: ");
    DEBUG_SERIAL.println(packet.payload);
    ...
    request->onReadyStateChange([request, packet](void *arg, AsyncHTTPRequest *req, int readyState) mutable {
      if (readyState == readyStateDone) {
        ...
        if (code != 200) {
          DEBUG_SERIAL.println("⚠ ERROR: Retry failed. Re-adding to queue.");
          RetryPacket newPacket = packet;
          newPacket.retryCount++;
          failedRequests.push_back(newPacket);
        } else {
          DEBUG_SERIAL.print("📥 Retry Server Response: ");
          DEBUG_SERIAL.println(req->responseText().substring(0, 100));  // ✅ แสดงแค่ 100 ตัว
        }
    ...
    if (request->readyState() == readyStateUnsent || request->readyState() == readyStateDone) {
        ...
        DEBUG_SERIAL.println("⚠ Request not ready to send (still in progress?)");
        delete request;
    }    
    yield();  // ป้องกัน Soft WDT reset
    delay(50);
  }
}
