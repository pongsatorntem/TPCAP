...
void loop()
{
  ...
  // ✅ พิมพ์เวลาแบบอ่านง่ายทุก 1 นาที (พร้อม offset UTC+7)
  ...
  if (millis() - lastReadableTsPrint > 60000) {
    ...
    printCurrentTimestampReadable(nowTs + 7 * 3600 * 1000);  // แสดงเวลาไทย
  }
  ...
  // ✅ รีบูตเวลา 06:00 และ 12:00 (UTC+7)
  static U64 lastAutoRebootTime = 0;

  U64 secondsInDay = ((nowTs / 1000) + 7 * 3600) % 86400;
  ...
  for (U1 cnt = 0; cnt < allSlotCount; cnt++) {
    ...
    if (slot->isSensorStateChanged(photoState, currentTimestamp)) {
      ...
      if (photoState == PHOTO_ON) {
        ...
        if (transferType == SLOT_RECEIVE_TYPE && slot->canPostNow() && activeRequests < MAX_ACTIVE_REQUESTS - 1 && slot->getPreviousSensorState() != PHOTO_ON &&
