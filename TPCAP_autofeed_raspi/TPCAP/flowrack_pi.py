...
def build_argparser():
    ...
    ap.add_argument("--station-code", type=str, default="9")
    ap.add_argument("--dry-run", action="store_true", help="print payloads instead of POST")
    ap.add_argument("--wifi-iface", default="wlan0")   # <== HERE
...
async def main():
    global ingest_q, _main_loop, _pending_hp, current_station_codeck
    ...
    async with aiohttp.ClientSession(connector=conn, timeout=timeout) as session:
        ...
        try:
            ...
            if not mapping:
                log("⚠️ Empty/invalid mapping for initial station "
                    f"{current_station_code}; will wait for ESP hp and hot-reload.")
            else:
                slots.clear()
            ...
            log("⚠️ Initial mapping load failed:", repr(e), 
                "— continuing; will wait for ESP hp and hot-reload.")
